...
# CGE Enhanced Website Builder using Bridge Agent
...
PROMPTS_FILE="/Users/royaltyvixion/Documents/cge software/cge-enhanced-website/ENHANCED_PROMPTS.md"
PROJECT_PATH="/Users/royaltyvixion/Documents/cge software/cge-enhanced-website/project"
START_FROM="${1:-Phase 1, Prompt 1}"  # Default to start from beginning
...
echo ""
echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   🚀 CGE Enhanced Website Builder - Bridge Agent Auto     ║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -d "$PROJECT_PATH" ]; then
    ...
    if [ ! -d "$PROJECT_PATH/.git" ]; then
        ...
        cd "$PROJECT_PATH"
        git init
        echo "# CGE Enhanced Website" > README.md
        echo "node_modules/" > .gitignore
        echo ".next/" >> .gitignore
        echo "dist/" >> .gitignore
        echo ".env.local" >> .gitignore
        git add .
        git commit -m "Initial commit - CGE Enhanced Website"
        cd - > /dev/null
...
if [ $? -eq 0 ]; then
    echo ""
    echo -e "${GREEN}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║        ✅ Enhanced website build completed successfully!   ║${NC}"
    echo -e "${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
